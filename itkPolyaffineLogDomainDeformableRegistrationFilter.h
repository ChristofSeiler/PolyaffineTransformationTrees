#ifndef __itkPolyaffineLogDomainDeformableRegistrationFilter_h
#define __itkPolyaffineLogDomainDeformableRegistrationFilter_h
...
/**
 * \class PolyaffineLogDomainDeformableRegistrationFilter
 ...
 *
 * PolyaffineLogDomainDeformableRegistrationFilter is a base case for filter implementing
...
class ITK_EXPORT PolyaffineLogDomainDeformableRegistrationFilter : 
    public DenseFiniteDifferenceImageFilter<TField,TField>
{
public:
  /** Standard class typedefs. */
  typedef PolyaffineLogDomainDeformableRegistrationFilter    Self;
  ...
  itkTypeMacro( PolyaffineLogDomainDeformableRegistrationFilter, 
  ...
  typedef typename VelocityFieldType::Pointer     VelocityFieldPointer;
    ...
    typedef typename PolyaffineTreeTransformType::Pointer PolyaffineTreeTransformPointer;
  ...
  typedef PDEDeformableRegistrationFunction<FixedImageType,MovingImageType,
	...
	static const unsigned int Dim = Superclass::ImageDimension;
	static const unsigned int DimHom = 4;
	
	typedef double MatrixElementType;
	typedef itk::MatrixOffsetTransformBase<MatrixElementType,Dim,Dim> MatrixTransformType;
	typedef itk::Matrix<MatrixElementType,DimHom,DimHom> MatrixType;
	typedef itk::Image<MatrixType,Dim> MatrixImageType;
	typedef typename MatrixImageType::Pointer MatrixImagePointer;
	typedef itk::Matrix<MatrixElementType,DimHom,1> CoorHomMatrixType; 
	typedef itk::Image<CoorHomMatrixType,Dim> CoorHomImageType;
	typedef typename CoorHomImageType::Pointer CoorHomImagePointer; 
	typedef itk::Matrix<MatrixElementType,Dim,1> VelocityHomMatrixType;
	typedef itk::Image<VelocityHomMatrixType,Dim> VelocityHomImageType;
	typedef typename VelocityHomImageType::Pointer VelocityHomImagePointer;
  ...
  void SetMui(vnl_matrix< MatrixElementType > mui) { m_Mui = mui; };
  void SetSigmaInverse(vnl_matrix< MatrixElementType > SigmaInverse) { m_SigmaInverse = SigmaInverse; };  
//    void SetLambdaZero(vnl_matrix< MatrixElementType > LambdaZero) { m_LambdaZero = LambdaZero; };
//    void SetCZero(vnl_matrix< MatrixElementType > CZero) { m_CZero = CZero; }
    
//    vnl_matrix< MatrixElementType > GetLambdaNew() { return m_LambdaNew; };
//    vnl_matrix< MatrixElementType > GetCNew() { return m_CNew; };
//    vnl_matrix< MatrixElementType > GetSigmaNewInverse() { return m_SigmaNewInverse; }
    
    void SetExplainedVelocityField( VelocityFieldType * ptr )
    { m_ExplainedVelocityField = ptr; }
  ...
  itkGetConstMacro( MaximumKernelWidth, unsigned int );
    ...
    itkSetMacro( VarianceVelocityField, double );
    itkGetConstMacro( VarianceVelocityField, double );

    itkSetMacro( UnionMaskDilationRadius, double );
    itkGetConstMacro( UnionMaskDilationRadius, double );
  ...
  virtual double GetPreviousMetric() const {return 0;}
    
    itkSetMacro( StartLevel, unsigned int );
    itkGetConstMacro( StartLevel, unsigned int );

    itkSetMacro( EndLevel, unsigned int );
    itkGetConstMacro( EndLevel, unsigned int );
    
//    unsigned int GetCurrentLevel() { return m_CurrentLevel; }
	
	void SetFixedMaskImage(FixedMaskImagePointer fixedMaskImage) { m_FixedMaskImage = fixedMaskImage; }
	FixedMaskImagePointer GetFixedMaskImage() { return m_FixedMaskImage; }
	
	void SetMovingMaskImage(MovingMaskImagePointer movingMaskImage) { m_MovingMaskImage = movingMaskImage; }
	MovingMaskImagePointer GetMovingMaskImage() { return m_MovingMaskImage; }
	
    void SetPolyaffineTransform(PolyaffineTreeTransformPointer polyaffineTree) { m_PolyaffineTree = polyaffineTree; }
    void GetPolyaffineTransform() { return m_PolyaffineTree; }
		
//	vnl_matrix< MatrixElementType > ComputeCorrelationsBetweenRegions();
    
	/** This method is called before iterating the solution. */
	virtual void Initialize();
    
    static vnl_matrix< MatrixElementType > KroneckerProduct(vnl_matrix< MatrixElementType > A, vnl_matrix< MatrixElementType > B);
...
//    if(m_LevelElapsedIterations == 1)
//        std::cout << "New level = " << m_CurrentLevel << std::endl;

//    std::cout << "Total iterations = " << this->GetElapsedIterations() << " Level iterations = " << m_LevelElapsedIterations << " Previous metric = " << this->GetPreviousMetric() << " Metric = " << this->GetMetric() << std::endl;
    
//    if(m_LevelElapsedIterations > 1) {
//        
//        if( (this->GetPreviousMetric()-this->GetMetric())/this->GetMetric() < 0.01 ) {
//            
//            ++m_CurrentLevel;
//            m_LevelElapsedIterations = 0;
//            
//            if(m_CurrentLevel >= m_EstimateLevel)
//                return true;
//        
//        }
//        else {
//            ++m_LevelElapsedIterations;
//        }
//    }
//    else
//        ++m_LevelElapsedIterations;
  ...
  virtual void SmoothGivenField(VelocityFieldType * field, const double StandardDeviations[ImageDimension]);
	...
	void ComputeSigma(std::vector< vnl_vector_fixed< MatrixElementType,Dim > > regionCenters, std::vector< vnl_matrix_fixed < MatrixElementType,Dim,Dim > > regionSigmasInverse, std::vector< MatrixElementType > regionSigmasDet);
...
private:
  PolyaffineLogDomainDeformableRegistrationFilter(const Self&); //purposely not implemented
  ...
  FieldExponentiatorPointer m_InverseExponentiator;
	...
	FixedMaskImagePointer m_FixedMaskImage;
	MovingMaskImagePointer m_MovingMaskImage;
    
    VelocityFieldPointer m_ExplainedVelocityField;
	
    unsigned int m_StartLevel;
    unsigned int m_EndLevel;
    
//    unsigned int m_CurrentLevel;
//    unsigned int m_LevelElapsedIterations;
    
    PolyaffineTreeTransformPointer m_PolyaffineTree;
    
	MatrixImagePointer m_CoordinatesImage;
	CoorHomImagePointer m_CoorHomImage;

	//std::vector< WeightImagePointer > m_WeightImages;
    WeightImagePointer m_TotalWeightImage;
    //std::vector< WeightImagePointer > m_TotalWeightImages;
	//vnl_matrix< MatrixElementType > m_M;

    vnl_matrix< MatrixElementType > m_Mui;
    vnl_matrix< MatrixElementType > m_SigmaInverse;
    vnl_matrix< MatrixElementType > m_SigmaCheck;
//    vnl_matrix< MatrixElementType > m_LambdaZero;
//    vnl_matrix< MatrixElementType > m_CZero;
//    vnl_matrix< MatrixElementType > m_LambdaNew;
//    vnl_matrix< MatrixElementType > m_CNew;
//    vnl_matrix< MatrixElementType > m_SigmaNewInverse;
	
	FixedMaskImagePointer m_UnionMaskImage;
	
	vnl_matrix< MatrixElementType > m_BinaryMatrix;
	
	//WeightImagePointer m_AlphaWeightImage;

	typedef std::pair<typename WeightImageType::PixelType,typename WeightImageType::PixelType> PairType;
	struct sort_pred {
    ...
    double m_VarianceVelocityField;
    double m_UnionMaskDilationRadius;
...
#include "itkPolyaffineLogDomainDeformableRegistrationFilter.txx"
#endif
